<script lang="ts">
	...
	let { provider }: { provider: 'google' | 'apple' } = $props();

	const providers = [
		{
			name: 'google',
			icon: `<svg xmlns="http://www.w3.org/2000/svg" width="0.98em" height="1em" viewBox="0 0 256 262">
				<path
					fill="#4285F4"
...
{#each providers.filter((p) => p.name === provider) as { icon }}
	<Button
		variant="outline"
		class="w-full gap-2"
		onclick={async () => {
